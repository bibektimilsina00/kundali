...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.warn("FastAPI auth login proxy connection error, using local Python fallback:", e);
    }
    ...
    const outputJson = await new Promise<string>((resolve, reject) => {
      execFile(
        pythonExe,
        [scriptPath, "login", jsonInput],
        { cwd: targetApiDir, maxBuffer: 10 * 1024 * 1024 },
        (error, stdout, stderr) => {
          if (error) {
            console.error("Python auth login error:", stderr);
            return reject(error);
          }
          resolve(stdout.trim());
        }
