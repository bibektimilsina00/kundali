...
export async function POST(req: Request) {
  try {
    ...
    try {
      const res = await fetch(`${fastapiUrl}/v1/auth/signup`, {
        method: "POST",
      ...
      console.warn("FastAPI auth signup proxy connection error, using local Python fallback:", e);
    }
    ...
    const outputJson = await new Promise<string>((resolve, reject) => {
      execFile(
        pythonExe,
        [scriptPath, "signup", jsonInput],
        { cwd: targetApiDir, maxBuffer: 10 * 1024 * 1024 },
        (error, stdout, stderr) => {
          if (error) {
            console.error("Python auth signup error:", stderr);
            return reject(error);
          }
          resolve(stdout.trim());
        }
    ...
    console.error("API /api/v1/auth/signup error:", err);
    return NextResponse.json(
      { error: "Failed to process signup", details: err?.message || String(err) },
      { status: 500 }
    );
  }
}
